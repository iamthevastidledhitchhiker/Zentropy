...
class IAN(object):
    ...
    def analysis(self, train_data, test_data):
        ...
        with open('analysis/train_' + str(timestamp) + '.txt', 'w') as f:
            for sample, num in self.get_batch_data(aspects, contexts, labels, aspect_lens, context_lens, len(aspects), False, 1.0):
                aspect_atts, context_atts, pred_outcome, correct_pred = self.sess.run([self.aspect_atts, self.context_atts,self.predict, self.correct_pred], feed_dict=sample)
                for a, b, c, d in zip(aspect_atts, context_atts, pred_outcome, correct_pred):
                    a = str(a).replace('\n', '')
                    b = str(b).replace('\n', '')
                    f.write('%s\n%s\n%s\n%s\n' % (a, b, c, d))
